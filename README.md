...
- 🤖 **Multi-threaded Testing**: Parallel execution across multiple contact forms
- 🔒 **CAPTCHA Support**: Handles Turnstile CAPTCHA challenges
- 📧 **Email Notifications**: Sends failure alerts via SMTP
- 🎭 **Human-like Behavior**: Mimics human typing patterns and delays
- 🔄 **Retry Logic**: Configurable retry attempts on failures
- 🌐 **Browser Extensions**: Supports browser extensions for enhanced functionality
- 📊 **Detailed Logging**: Comprehensive error reporting and screenshots
...
|----------|-------------|---------|----------|
| `CONTACT_PAGES` | Comma-separated URLs of contact forms to test | None | ✅ Yes |
| `MAX_WORKERS` | Number of parallel test workers | 10 | ❌ No |
| `RERUNS_ON_FAILURE` | Number of retry attempts on test failure | 1 | ❌ No |
| `USE_HEADLESS` | Run browser in headless mode | True | ❌ No |
| `SMTP_HOST` | SMTP server hostname | None | ❌ No |
| `SMTP_PORT` | SMTP server port | 587 | ❌ No |
| `SMTP_USERNAME` | SMTP authentication username | None | ❌ No |
| `SMTP_PASSWORD` | SMTP authentication password | None | ❌ No |
| `EMAIL_FROM` | Sender email address | None | ❌ No |
| `EMAIL_TO` | Comma-separated recipient email addresses | None | ❌ No |
